      ...
      <ul>
        <transition-group name="list" enter-active-class="animated bounceInUp" leave-active-class="animated bounceOutUp">
          <li v-for="(data,index) in skills" v-bind:key="index">
            {{index + 1}}.{{data.skill}} 
            <i class="fa fa-minus-circle" @click="removeSkill(index)" style="color:red"></i>
            <i class="fa fa-pencil-square-o" aria-hidden="true" @click="editSkill(index)" style="color:green"></i>
          </li>
        </transition-group>
      </ul>
...
export default {
  ...
  methods: {
    ...
    editSkill(idx){
      this.skill = this.skills[idx].skill;
